...
This method ensures sensitive information is kept secure, while also making your code portable across different environments without exposing the keys.
...
# Define the salary sequence
salary_sequence <- seq(15000, 100500, by = 10000)

DATE_AGE_DATA <- add_date_and_age(combined_returns_2008, 45, "2008-05-01")
portfolio_data <- generate_portfolio_values(salary_sequence, DATE_AGE_DATA)



joined_data_TRS_ORP <- portfolio_data |>
  left_join(payout_results, by = c("salary" = "starting_salary")) |>
  rename("Starting Salary" = salary, "Total Portfolio Value, ORP" = total_portfolio_value, "Monthly Payout, TRS" = monthly_payout)

# joined_data_TRS_ORP, data to display
...
joined_data_TRS_ORP$"Total Portfolio Value, ORP"

monthly_withdrawal <- seq(1000,10000, by=1000)
monthly_withdrawal
...
oldest_age_data <- generate_portfolio_durations(portfolio_values, monthly_withdrawals, 61)

wide_data <- oldest_age_data |>
  pivot_wider(
    names_from = Monthly_Withdrawal,
    values_from = Duration,
    names_prefix = "Withdrawal "
  )
wide_data
```
</details>
...
Unfortunately, my app is unable to be hosted on [shinyapps.io](https://www.shinyapps.io/) because their servers [only support R version `>4.3.3`](https://forum.posit.co/t/unsupported-r-version-4-4-2-what-to-do/194760), while my machine is running a later version of **R**. In lieu of a live version of the app, here is screen-capture video of the app: 

[Youtube Link to demonstration of Shiny App](https://youtu.be/C7dv-c2ufHs)

In addition, what follows is a plot of a **Monte Carlo** simulation for `30` years of work and `10` years retirement: 

![](images/mp-04/portfolio-simulation.jpg){style="width:90%; box-shadow: -1px 1px 2px rgba(0, 0, 0, 0.5); display: block; margin: auto;"}
