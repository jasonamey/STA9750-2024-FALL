...
if (st_is_empty(dc_location$geometry)) {
  dc_location <- st_sf(STATENAME = "DISTRICT OF COLUMBIA",
                       winning_party = "D", 
...
ggplot() +
  ...
  scale_fill_manual(values = c("D" = "lightblue", "R" = "lightcoral")) + 
...
# Add abbreviations for state names using R state package. Handle
# Washington DC 
...
state_winner_take_all <- function(data_set, election_year, ec_summary_data) {
  ...
  ggplot(df_year_requested, aes(state = state_abbr, fill = winner_party)) +
    ...
    labs(
      title = paste("United States Electoral College Votes by State (", election_year, ")", sep = ""),
...
plot_electoral_votes <- function(data, election_year) {
  ...
  winner_party1 <- total_votes_list[[1]]$winner_party
  total_votes1 <- total_votes_list[[1]]$total_votes
  
  proper_name2 <- total_votes_list[[2]]$proper_name
  winner_party2 <- total_votes_list[[2]]$winner_party
...
electoral_votes_by_state <- ELECTION_DATA_HOUSE |> 
  group_by(year, state) |>
  summarize(votes = n_distinct(district) + 2)
  
electoral_votes_by_state

state_percentages <- ELECTION_DATA_PRESIDENT |>
...
state_percentages_w_votes <- state_percentages |>
  left_join(electoral_votes_by_state, by = c("year", "state")) |>
  rename(electoral_votes = votes)
  
state_electoral_votes <- state_percentages_w_votes |>
  group_by(year, state, state_po) |>
  mutate(
...
state_electoral_votes <- state_electoral_votes |>
...
plot_electoral_votes <- function(data, election_year) {
  ...
  summary <- df_year_filtered |> 
    group_by(proper_name, party_detailed) |> 
  ...
  total_votes1 <- total_votes_list[[1]]$total_electoral_votes
  
  proper_name2 <- total_votes_list[[2]]$proper_name
  winner_party2 <- total_votes_list[[2]]$party_detailed
  total_votes2 <- total_votes_list[[2]]$total_electoral_votes
...
plot_electoral_votes(state_electoral_votes, 1988)
...
<!-- # Sample data for electoral votes by state for 2020 (replace with actual data) -->
...
<!--   votes = c(9, 3, 11, 6, 55, 9, 7, 3, 3, 29, 16, 4, 4, 20, 11, 6, 6, 8, 8, 4, -->
<!--             10, 11, 16, 10, 6, 10, 3, 5, 6, 4, 14, 5, 29, 15, 3, 18, 7, 7, 20, -->
<!--             4, 9, 3, 11, 38, 6, 3, 13, 12, 5, 10, 3) -->
<!-- ) -->


<!-- ggplot(electoral_votes, aes(state = state, fill = votes)) + -->
<!--   statebins::geom_statebins(radius = grid::unit(0.1, "cm")) + -->
<!--   scale_fill_gradient(low = "red", high = "darkblue", name = "Electoral Votes") + -->
<!--   labs(title = "United States Electoral College Votes by State") + -->
<!--   theme_minimal() + -->
<!--   theme( -->
<!--     legend.position = "right", -->
<!--     panel.background = element_blank(), -->
